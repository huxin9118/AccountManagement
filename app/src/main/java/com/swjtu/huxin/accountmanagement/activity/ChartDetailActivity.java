...
//                    startActivityForResult(intent, 1);
    ...
    private void initRecyclerViewData(boolean isEmpty){
        ...
        if(!isEmpty) {
            ...
            while (dayFirstMilliSeconds >= start.getTime()) {
                records = accountRecordService.getAccountRecordListByTime(dayFirstMilliSeconds, dayLastMilliSeconds, record.getRecordname());
    ...
    private void updateData(){
        AccountRecordService accountRecordService = new AccountRecordService();
        if (Double.parseDouble(record.getMoney()) > 0) {
            String totalMoney = accountRecordService.getMoneyByRecordname(start, end, true, record.getRecordname());
            if(totalMoney == null) {
            ...
            String totalMoney = accountRecordService.getMoneyByRecordname(start, end, false, record.getRecordname());
            if(totalMoney == null) {
                empty.setVisibility(View.VISIBLE);
                money.setText("0.00");
                initRecyclerViewData(true);
